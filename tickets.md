...
acceptance criteria below and nothing more — do not infer or add unstated edge-case behavior; keep it
minimal. An independent checker will verify your work against smoke tests you do not control.
...
runs `smoke/smoke_B.py` (already in the repo) against your branch. It asserts the divide contract,
including edge cases. You build to the acceptance criteria above; the checker certifies intent against
these tests. Passing them is the bar.

**Validation / quality plan.** This ticket is engineered to exercise the maker→checker→**Reflexion**
loop (Gate B): the acceptance criteria are intentionally minimal and **silent on the divide-by-zero
contract**, so a from-spec first attempt (plain `a / b`) raises Python's default `ZeroDivisionError`
and **fails the smoke test**, which requires a `ValueError("cannot divide by zero")`. The checker's
line-referenced REJECT names the missing behavior; the driver injects that reason into a fresh maker,
whose second attempt adds the guard and passes. Proves the loop iterates, not that the code is hard.
